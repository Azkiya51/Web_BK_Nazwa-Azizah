...
async function loadXBanner() {
    ...
    try {
        ...
        data.forEach(item => {
            ...
            const card = document.createElement('div');
            card.className = 'xbanner-card';
            card.innerHTML = `<img src="${imgUrl}" alt="${item.judul || 'X-Banner BK'}" class="xbanner-img" loading="lazy">`;
            card.addEventListener('click', () => openXBannerLightbox(imgUrl, item.judul || ''));
            grid.appendChild(card);
        });
...
function openXBannerLightbox(src, alt) {
    document.getElementById('xbannerLightboxImg').src = src;
    document.getElementById('xbannerLightboxImg').alt = alt;
    document.getElementById('xbannerLightbox').classList.add('active');
    document.body.style.overflow = 'hidden';
}

function closeXBannerLightbox(e) {
    if (e.target === document.getElementById('xbannerLightbox')) {
        document.getElementById('xbannerLightbox').classList.remove('active');
